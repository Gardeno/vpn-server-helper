...
def main():
    ...
    if request.method == 'POST':
        ...
        if not path.exists(path_to_output_openvpn_config):
            try:
                key_process = subprocess.Popen(['./easyrsa', 'gen-req', client_name, 'nopass', 'batch'], cwd=PATH_TO_EASY_RSA)
                key_process.wait()
            ...
            try:
                sign_process = subprocess.Popen(['./easyrsa', 'sign-req', 'client', client_name, 'batch'], cwd=PATH_TO_EASY_RSA)
                sign_process.wait()
